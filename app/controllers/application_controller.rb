class ApplicationController < ActionController::Base
  ...
  def handle_exception(e)
    ...
    unless performed?
      respond_to do |format|
        format.html { render text: "Something went wrong. We are looking into it. Error details: #{e.class}, #{e.message}", status: 500 }
        format.json { render json: {msg: "Something went wrong. We are looking into it. Error details: #{e.class}, #{e.message}"}, status: 500 }
      end
    end
  end
